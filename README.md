...
큐레이션 DB, 가격/리뷰는 실시간 조회로 가져오는 **하이브리드** 구조.

지금은 실제로 빌드·실행·테스트까지 확인된 상태다. 카테고리 5개(우유/생수/통조림햄/두부/참치캔)
샘플 데이터가 들어 있고, 3개 툴(`list_categories`/`search_products`/`compare_products`) 전부
실제로 curl로 호출해서 정상 동작 확인했다. (2026-08-24 갱신)
...
src/
  ...
  lib/liveData.ts        # 가격/리뷰 실시간 조회 - 네이버쇼핑 연동 코드 있음, 키 없으면 null
...
- **가격/리뷰는 실제로 안 붙어 있다.** `lib/liveData.ts`에 네이버쇼핑 검색 API 연동 코드는
  작성해뒀지만, `.env`에 `NAVER_SHOPPING_CLIENT_ID`/`SECRET`이 없으면 (지금 이 상태) 여전히
  `null`을 반환한다. 키를 발급받아 `.env`에 넣으면 코드는 바로 동작하는데, **실제 키로 첫 호출
  검증은 아직 안 됨** — API 스펙은 공식 문서를 직접 못 열어봐서 3rd-party 블로그 교차 확인으로
  짜맞춘 것. 楽天市場(JP) 연동은 아직 코드도 없음 (JP 카테고리 제품이 없어서 후순위로 미룸).
...
## 5. 배포 (Render, GitHub 연동만으로 가능)

`Dockerfile` / `.dockerignore` / `render.yaml`을 이미 만들어뒀다. 계정이 없어도 GitHub 계정으로
바로 가입할 수 있어서 Cloud Run/Fly.io보다 진입장벽이 낮음.

1. 이 저장소를 GitHub에 올린다 (아직 원격 저장소가 없으면 GitHub에서 새 repo 만들고):
   ```bash
   git remote add origin https://github.com/<내계정>/mart-compare-mcp.git
   git push -u origin main
   ```
2. https://render.com 접속 → "Get Started" → GitHub 계정으로 가입/로그인
3. New → Web Service → 방금 올린 GitHub repo 선택
4. Render가 `render.yaml`을 자동으로 인식함 (Blueprint). Runtime은 Docker로 잡힘 — 그대로 진행
5. 환경변수 입력 화면에서 `.env`에 있는 값(FOODSAFETY_API_KEY 등) 채우기 — `render.yaml`에
   `sync: false`로 걸어둬서 Render가 값을 직접 물어봄 (레포에 키가 올라가지 않도록)
6. 배포 완료되면 `https://mart-compare-mcp-xxxx.onrender.com/health` 로 헬스체크,
   `https://mart-compare-mcp-xxxx.onrender.com/mcp` 가 PlayMCP에 등록할 엔드포인트
7. 무료 플랜은 트래픽 없으면 슬립 상태로 들어가고 첫 요청에 콜드스타트(수십 초)가 있을 수 있음 —
   실사용 트래픽이 생기면 유료 플랜 전환 고려
...
- [x] 네이버쇼핑 연동 코드 작성 (키 없어서 실제 호출 미검증)
- [x] Dockerfile/render.yaml 작성
- [ ] 네이버쇼핑 API 키 발급 → 실제 호출 검증
- [ ] 楽天市場 API 키 발급 → JP 제품 가격 채우기 (JP 제품군 자체도 아직 1개뿐이라 우선순위 낮음)
- [ ] GitHub repo 생성 + Render 가입 + 배포 실행 (계정 필요 — 직접 진행 필요)
