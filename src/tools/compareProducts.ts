...
export function registerTools(server: McpServer) {
  ...
  server.registerTool(
        "list_categories",
    {
            ...
            description:
                      "[마트 제품 비교] 현재 DB에 등록된 제품 카테고리 목록을 반환한다 (예: milk, water, canned-ham). 사용자가 어떤 카테고리를 비교할 수 있는지 모를 때 먼저 호출.",
            inputSchema: {},
            annotations: {
                      title: "비교 가능한 제품 카테고리 목록",
                      readOnlyHint: true,
  ...
  server.registerTool(
        "search_products",
    {
            title: "카테고리 내 제품 검색",
            description:
                      "[마트 제품 비교] 카테고리(및 선택적으로 국가, 검색어)로 제품 목록을 찾는다. compare_products에 넘길 id를 확인할 때 사용.",
            inputSchema: {
  ...
  server.registerTool(
        "compare_products",
    {
            title: "제품 N개 비교",
            description:
                      "[마트 제품 비교] product_ids로 넘긴 제품들의 스펙(원산지/인증/영양정보)과 실시간 가격/리뷰 정보를 합쳐서 반환한다. " +
